...
export const NoCodeConfigComponent = (props: NoCodeConfigComponentProps) => {
  ...
  const canRender = (conditions: EditorPropertyCondition[] = []): boolean => {
    let satisfied = conditions.map((condition) => {
      let targetValue = getValueFromOptions(condition.key, condition.outsideJSON);
  ...
  const propsWithoutGroup = (editorProps.properties?.filter((p) => !p.group) || []).filter((p) => {
  ...
  uniq(editorProps.properties?.map((p) => p.group))
    .filter(Boolean)
    .forEach((g) => {
      const props = (editorProps.properties?.filter((p) => p.group === g) || []).filter((p) => {
        return p.showIf && p.showIf.length > 0 ? canRender(p.showIf) : true;
      });
      if (props.length > 0) {
                      ...
                      <>
                        {prop.options && prop.options.length > 0 ? (
                          ...
                          <TextArea
                            ...
                            value={
                              getValueFromOptions(prop.key, prop.outsideJSON) || (prop.type === 'number' ? '0' : '')
                            }
                            onChange={(e) =>
